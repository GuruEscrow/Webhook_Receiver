 ...
 */
@WebServlet("/indianuat/confirmation")
public class IndianBankLiveConfirmationWebhook extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public IndianBankLiveConfirmationWebhook() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			System.out.println("Updation Requested body:");
			System.out.println(prettyJson);

			// Validating the required fields are there or not
			PrintWriter out = response.getWriter();
			if (!(jsonNode.has("VirtualAccountNo") && jsonNode.has("Amount") && jsonNode.has("BankTxnRefNo")
					&& jsonNode.has("UTR_Reference") && jsonNode.has("Txndate") && jsonNode.has("Txntimestamp")
					&& jsonNode.has("RemiiterAccountNo") && jsonNode.has("RemitterIFSCode"))) {
				response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
				String res = "{"
						+ " \"message\":\"VirturalAccoutnNo, Amount, BankTxnRefNo, UTR_Reference, Txndate, RemiiterAccountNo, RemitterIFSCode and Txntimestamp are required field\""
						+ "}";
